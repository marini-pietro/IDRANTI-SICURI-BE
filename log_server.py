...
class ConditionalFormatter(logging.Formatter):
    """
    Formatter that includes the timestamp only when record.include_timestamp is True.

    The format always contains the levelname; the asctime portion is toggled per-record.
    """

    def __init__(self, fmt_with_time: str, fmt_without_time: str, datefmt: str | None = None):
        super().__init__(fmt_with_time, datefmt=datefmt)
        self._fmt_with_time = fmt_with_time
        self._fmt_without_time = fmt_without_time

    def format(self, record: logging.LogRecord) -> str:
        original_fmt = self._style._fmt
        try:
            if getattr(record, "include_timestamp", False):
                self._style._fmt = self._fmt_with_time
            else:
                self._style._fmt = self._fmt_without_time
            return super().format(record)
        finally:
            self._style._fmt = original_fmt

    def formatTime(self, record: logging.LogRecord, datefmt: str | None = None) -> str:
        # Include milliseconds with a comma separator like 2026-04-16 14:05:30,017
...
class Logger:
    ...
    def __init__(self, log_file: str, console_level: int, file_level: int) -> None:
        ...
        # The format always includes the level name; the ConditionalFormatter
        # will include the asctime portion only when a record sets
        # `include_timestamp=True` via the `extra` parameter.
        local_logs_fmt = "%(asctime)s - %(levelname)s - %(message)s" # format for logs generated by the log server itself
        remote_logs_fmt = "%(levelname)s - %(message)s" # format for logs coming from logging interfaces 
        # (the final timestamp in logs originating from logging interfaces will be the one included in the message content
        # coming from the log interfaces themselves, as that is the timestamp of the actual log event, not the time of reception by the log server
        formatter = ConditionalFormatter(local_logs_fmt, remote_logs_fmt, datefmt="%Y-%m-%d %H:%M:%S")

        # Set the formatter for both handlers (logs in console and in file will have the same format)
    ...
    def log(
        ...
        # Delegate formatting to the logging system; provide a flag via
        # `extra` so the ConditionalFormatter can figure out if the timestamp should be included.
        formatted_message = f"[{origin}] {message}"
        log_method = getattr(self.logger, log_type)
        log_method(formatted_message, extra={"include_timestamp": include_timestamp})
...
def start_syslog_server(host: str, port: int) -> None:
    ...
    try:
        ...
        logger.log(
            log_type="info",
            message="Shutting down syslog server...",
            origin=LOG_SERVER_IDENTIFIER,
            include_timestamp=True,
        )
